...
void ahp_xc_scan_crosscorrelations(correlation *crosscorrelations, double *percent, int *interrupt)
{
    ...
    memset(crosscorrelations, 0, sizeof(correlation)*(unsigned int)(ahp_xc_get_delaysize()*2+1)*(unsigned int)ahp_xc_get_nbaselines());
    for(index1 = 0; index1 < ahp_xc_get_nlines(); index1++)
        ahp_xc_set_delay(index1, 0);
    for(index1 = 0; index1 < ahp_xc_get_nlines(); ) {
        for(i = ahp_xc_get_delaysize()-1; i >= 0; i --) {
            if(*interrupt)
                goto stop;
            if(!ahp_xc_get_packet(data1, NULL, data2)) {
                ahp_xc_set_delay(index1, i+1);
                for(index2 = 0; index2 < ahp_xc_get_nlines(); index2++) {
                    ...
                    int index = ((index1>index2?-i:i)+ahp_xc_get_delaysize()+(ahp_xc_get_delaysize()*2+1)*idx);
                ...
                (*percent) += 100.0 / (ahp_xc_get_delaysize()*ahp_xc_get_nlines());
                index1++;
            }
        }
    }
...
void ahp_xc_scan_autocorrelations(correlation *autocorrelations, double *percent, int *interrupt)
{
    ...
    memset(autocorrelations, 0, sizeof(correlation)*(unsigned int)ahp_xc_get_delaysize()*(unsigned int)ahp_xc_get_nlines());
    int index = 0;
