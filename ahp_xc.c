            ...
            for(y = 1; y < order && !*interrupt; y++) {
                ...
                while(k < lines[0].len/lines[0].step) {
                    ...
                    if(ts0 == 0.0)
                        ts0 = ts;
                    ts -= ts0;
                    ahp_xc_get_crosscorrelation(&correlations[o], inputs, order, packet, -ts);
                    wait_no_threads();
                    o++;
                    k++;
                }
...
void ahp_xc_set_channel_cross(uint32_t index, off_t value, size_t size, size_t step)
{
    ...
    ahp_xc_set_test_flags(index, ahp_xc_get_test_flags(index)|0x10);
    ...
    ahp_xc_set_test_flags(index, ahp_xc_get_test_flags(index)|0x20);
    len = (((int)log2(ahp_xc_get_delaysize()) & 3) + 4) >> 2;
    ahp_xc_send_command(SET_DELAY, (unsigned char)(len&0xf));
    for(idx = 0; idx < len; idx ++) {
