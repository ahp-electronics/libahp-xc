...
{
    read_argument *data = (read_argument *)arg;
    data->size = 0;
    ahp_xc.threads_running = 1;
    while(data->threads_running) {
        data->threads_running = 1;
        int nread;
        if(pthread_mutex_trylock(&data->mutex)) {
            memset(data->buffer, 0, 0x1000000);
            data->size = 0;
            while(1) {
                unsigned char c;
                nread = ahp_serial_RecvBuf(&c, 1);
                if(nread > 0) {
                    *((char*)(data->buffer+data->size)) = c;
                    data->size ++;
                }
                if(c == '\r') {
                    memcpy(data->buffer+data->size, data->buffer+data->size+1, nread-1);
...
int32_t ahp_xc_connect_fd(int32_t fd)
{
    ...
    ahp_xc.delaysize = 0;
    ahp_xc.frequency = 0;
    ahp_xc.packetsize = 1024;
    ahp_xc.rate = R_BASE;
    if(fd > -1) {
        ahp_xc.connected = 1;
        ahp_xc.buf = (char*)malloc(1);
        ahp_xc.detected = 0;
...
void ahp_xc_disconnect()
{
    if(ahp_xc.connected) {
        ...
        if(ahp_xc.mutexes_initialized) {
            ...
            pthread_mutex_destroy(&ahp_xc.read_data.mutex);
...
sattic int32_t ahp_xc_scan_autocorrelations(ahp_xc_scan_request *lines, uint32_t nlines, ahp_xc_sample **autocorrelations, int32_t *interrupt, double *percent)
{
    if(!ahp_xc.detected) return 0;
    int32_t r = -1;
    uint32_t i = 0;
...
int32_t ahp_xc_set_capture_flags(xc_capture_flags flags)
{
    ...
    ahp_xc.read_data.capturing = flags & CAP_ENABLE;
